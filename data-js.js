...
function showViewList() {
  ...
  // 過濾資料
  const filteredData = selectedCategory === "全部" ? data : data.filter((item) => item[0] === selectedCategory)
  ...
  function renderSingleView() {
    ...
    slider.addEventListener("input", (e) => {
      if (isRandomMode) {
        currentIndex = randomIndices[Number.parseInt(e.target.value)]
      } else {
        currentIndex = Number.parseInt(e.target.value)
      }
      renderCard()
    })
  ...
  function renderCard() {
    ...
    if (card) {
      ...
      const currentValue = existingSlider
        ? existingSlider.value
        : isRandomMode
          ? randomIndices.indexOf(currentIndex)
          : currentIndex
  ...
  function createItemElement(item, index) {
    ...
    indexElement.textContent = `${index + 1}${!isGroupMode ? ` / ${filteredData.length}` : ""}`
    ...
    questionElement.textContent = item[questionLangIndex]

    // 答案元素 - 將在新的一行顯示
    const answerElement = document.createElement("div")
    answerElement.className = "item-answer clickable"
    answerElement.textContent = item[answerLangIndex]
    ...
    // 將問題容器和答案添加到項目中
    itemElement.appendChild(questionContainer)
